...
############# == main functions == #####################
    ...
    df_sim_mole = calculate_mole_similarity(df_seg_mole)
...
@pandas_udf(DoubleType(),PandasUDFType.SCALAR)
def calculate_mole_similarity_after_seg(standard_mole,mole_cut_words,mole_name):
    frame = {"standard_mole":standard_mole,
            "mole_cut_words":mole_cut_words,
            "mole_name":mole_name}
    df = pd.DataFrame(frame)
    ...
    df['mole_sim'] = df.apply(lambda x: calculate_similarity(x.standard_mole, x.mole_cut_words,x.mole_name), axis=1)
    return df['mole_sim']

##### == calulate_similarity == #######
def calculate_mole_similarity(df_seg_mole):
    
    df_seg_mole = df_seg_mole.withColumn("eff_mole",calculate_mole_similarity_after_seg(df_seg_mole.MOLE_NAME_STANDARD,\
                                                                                        df_seg_mole.MOLE_CUT_WORDS,\
                                                                                       df_seg_mole.MOLE_NAME))
    return df_seg_mole
