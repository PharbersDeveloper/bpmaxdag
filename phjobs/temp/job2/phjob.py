...
#     weicai = spark.read.parquet("s3a://ph-max-auto/2020-08-11/BPBatchDAG/refactor/zyyin/eia/raw_data_2")
    qilu.select("SPEC").distinct().show(500)
    qilu.repartition(1).write.mode("overwrite").csv("s3a://ph-max-auto/2020-08-11/data_matching/temp/mzhang/cpa_source_table/qilu_csv",header=True)
    print(qilu.printSchema())
    print(qilu.count())
    print("ok!")
    return {}
