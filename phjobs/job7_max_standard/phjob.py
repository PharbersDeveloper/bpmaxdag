...
def execute(a, b):
    ...
    max_standard_brief.show()
    
    '''
    # 根据日期分桶写出
    max_standard_all = max_standard_all.repartition("Date")
    max_standard_all.write.format("parquet").partitionBy("Date") \
    ...
    # 用于提数的max结果文件path
    path_for_extract_path = "s3a://ph-stream/common/public/max_result/0.0.5/path_for_extract.csv"
    ...
    for eachpath in path_all_brief:
        file = spark.read.parquet(eachpath)
        if index ==0:
            max_standard_brief = file
        else:
            max_standard_brief = max_standard_brief.union(file)
        index += 1
    ...
    # 筛选
    max_filter_list = max_standard_brief.where((max_standard_brief.Date >= time_left) & (max_standard_brief.Date <= time_right)) \
            .where(max_standard_brief.Molecule.isin(Molecule))
    
    def get_project_date_func(df):
        import pandas as pd
        Date_list = df["Date"].tolist()
        Date_list = str(Date_list)
        project = df["project"][1]
        return pd.DataFrame([[project] + [Date_list]], columns=["project", "Date_list"])
        
    schema = StructType([
            StructField("project", StringType(), True), 
            StructField("Date_list", ArrayType(StringType()), True)
            ])
    get_project_date = pandas_udf(get_project_date_func, schema, PandasUDFType.GROUPED_MAP)
    
    df_out = max_filter_list.groupby(["project"]).apply(get_project_date)
