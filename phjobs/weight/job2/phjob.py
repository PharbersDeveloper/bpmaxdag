...
universe = universe.select("Panel_ID", "Province", "City") \
...
def func_target_weight(pdf, dict_target_share, l, m, lmda, gradient_type):
    ...
    H3 = deepcopy(H2)  
    H3 = H3[['City', 'PHA', 'City_Sample', 'weight', 'Bedsize>99','weight_factor', 
            'weight_factor1', 'W']]    
    return H3
...
for market in market_list:
    ...
    out = data_target.groupBy('City').apply(udf_target_weight).persist()
    ...
    # 4. 输出结果整理
    # 4.1 原始的weight结果
    df_weight_out = out.select('City', 'PHA', 'weight', 'Bedsize>99', 'weight_factor', 'weight_factor1', 'W')
    ...
    # 4.3 share 和 gr 结果
    data_final = data_target.join(out.select('PHA','W'), on='PHA', how='left')
...
# 输出最终结果到
'''
weight = spark.read.parquet(df_weight_tmp_path)
weight = weight.repartition(1)
weight.write.format("parquet") \
    .mode("overwrite").save(weight_path)
'''
