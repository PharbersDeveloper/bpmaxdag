...
def execute(**kwargs):
    ...
    spark = SparkSession.builder \
        .master("yarn") \
        .appName("datamart original dim chc_tag job") \
...
#  |-- version: string (nullable = true)
    ...
    dest = reading.select("TAG").distinct().withColumn("_ID", monotonically_increasing_id())
    for one in dest.collect():
        print(one)
    # dest.show(100)
    # logger.info(dest.count()) # 131
