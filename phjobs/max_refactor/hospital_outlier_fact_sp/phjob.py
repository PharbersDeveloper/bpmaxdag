...
extract_outlier_universe -> hospital_outlier_fact （只包含SEG与PANEL）
...
def execute(**kwargs):
    ...
    _label = kwargs["label"]
    _hosp_outlier_fact_output = kwargs["hosp_outlier_fact_output"]
    
    outlier_fact_mapping = [
        {
            "CATEGORY": _label,
            "TAG": "SEG",
            "COLUMN": "SEG"
    ...
    def fact_table(item):
        fact = dim.selectExpr("ID as HOSPITAL_ID", "PHA_ID AS DIM_PHA_ID") \
            ...
            .withColumn("LABEL", lit(_label)) \
            .drop(item["COLUMN"])
        return fact

    fact_un_all = reduce(lambda x, y: x.union(y), list(map(fact_table, outlier_fact_mapping)))
    fact_un_all \
        .selectExpr("ID", "HOSPITAL_ID", "CATEGORY", "TAG", "VALUE", "COMPANY", "VERSION", "LABEL") \
        .write \
        .partitionBy("COMPANY", "VERSION") \
        .parquet(_hosp_outlier_fact_output, "append")
