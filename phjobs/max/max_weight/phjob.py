...
def execute(**kwargs):
    ...
    from pyspark.sql.functions import col# 测试输入
g_project_name = "贝达"
g_market = 'BD1'
...
time_left = "202001"
time_right = "202012"
out_dir = "202012_test"logger.debug('数据执行-start：max放大')
...
if g_use_d_weight:
    p_PHA_weight_default = max_path + "/" + g_project_name + '/PHA_weight_default'# # =========== 数据准备，测试用 =============
# 1、panel 文件
...
df_original_panel = df_original_panel.where((col('MARKET') == g_market) & 
                                      (col('DATE') >= time_left) & (col('DATE') <= time_right)).cache()# 2.医院权重文件	 
...
df_PHA_weight_market = df_PHA_weight.where(df_PHA_weight.MARKET == g_market)# 3. universe 文件
...
df_panel_seg_weight = df_panel_seg_weight.withColumnRenamed('PROVINCE_WEIGHT', 'PROVINCE') \
                .withColumnRenamed('CITY_WEIGHT', 'CITY')# 将非样本的segment和factor等信息合并起来：get_uni_with_factor
...
# 合并样本部分
df_max_result = df_max_result.union(df_panel.select(df_max_result.columns))# 输出结果
...
logger.debug('数据执行-Finish')# df_max_result.agg(func.sum('Predict_Sales'), func.sum('Predict_Unit')).show()# df=spark.read.parquet('s3a://ph-max-auto/v0.0.1-2020-06-08/贝达/202012_test/MAX_result/MAX_result_202001_202012_BD1_hosp_level/')
