...
def execute(**kwargs):
    ...
    output_mole_mapping_table_parquet = result_path_prefix + kwargs["correct_mole_mapping_table_parquet"]
    output_mole_mapping_table_csv = result_path_prefix + kwargs["correct_mole_mapping_table_csv"]
    output_mole_not_in_table = result_path_prefix + kwargs["mole_not_exist_in_standard"]
    ########### == output == ###########
    ...
    df_mole_not_in_standard_info = get_mole_not_exist_in_standard(input_df=df_correct,\
                                                           input_col="MOLE_NAME_STANDARD")
    df_mole_in_standard = get_mole_in_standard(input_df=df_correct,\
                                                 input_col="MOLE_NAME_STANDARD")
    
    df_mole_mapping_table_info =  get_mole_mapping_table(input_dataframe=df_mole_in_standard\
                                     ,input_mole="MOLE_NAME"\
                                     ,input_standard_mole="MOLE_NAME_STANDARD"\
                                     ,input_master="MASTER_MOLE")
    
    df_mole_mapping_table_csv_info = make_array_into_string(input_dataframe=df_mole_mapping_table_info[0],\
                                                      input_col="MASTER_MOLE")   
    ########## === RESULT === #############
    #### 标准表缺失数据
    write_files(input_df_info=df_mole_not_in_standard_info,\
                path_output=output_mole_not_in_table)
    ### 正确的mapping表
    write_files(input_df_info=df_mole_mapping_table_info,\
                path_output=output_mole_mapping_table_parquet)
    
    write_files(input_df_info=df_mole_mapping_table_csv_info,\
                path_output=output_mole_mapping_table_csv)
...
def loading_parquet_files(spark, input_path):
    try:
        ...
        df = df.select("PACK_ID_CHECK","MOLE_NAME_STANDARD","MOLE_NAME")
        return df
    except:
...
###### == 获取不在标准表中的数据 == ########
def get_mole_not_exist_in_standard(input_df,input_col):
...
###### == 获取有效数据 == ########
def get_mole_in_standard(input_df,input_col):
...
def get_mole_mapping_table(input_dataframe,input_mole,input_standard_mole,input_master):
    ...
    data_frame =  input_dataframe.groupBy(col(input_standard_mole))\
                                .agg(collect_set(col(input_mole)).alias(input_master))
    data_frame.show(200)
    df_type = "parquet"
