...
def execute(**kwargs):
    ...
    extract_file = kwargs["extract_file"]
    ...
    if data_type != "max" and data_type != "raw":
        logger.error('wrong input: data_type, max or raw')
    ...
    ims_sales_path = max_path + \
        "/Common_files/extract_data_files/cn_IMS_Sales_Fdata_202007.csv"
    molecule_ACT_path = max_path + \
        "/Common_files/extract_data_files/product_map_all_ATC.csv"
    packID_ACT_map_path = max_path + \
        "/Common_files/extract_data_files/packID_ATC_map.csv"
    ...
    out_extract_data_path = out_path + "/" + outdir + \
        "/out_" + timenow + "_" + out_suffix + '.csv'
    ...
    molecule_ACT = molecule_ACT.select(
        "MOLE_NAME_EN", "MOLE_NAME_CH").distinct()
    packID_ACT_map = packID_ACT_map.select(
        "MOLE_NAME_EN", "MOLE_NAME_CH").distinct()
    ...
    ims_sales = ims_sales.join(
        molecule_name_map, on="MOLE_NAME_EN", how="left").distinct()

    if atc and max([len(i) for i in atc]) == 3:
        ims_sales = ims_sales.withColumn(
            "ATC", func.substring(ims_sales.ATC, 0, 3)).distinct()
    elif atc and max([len(i) for i in atc]) == 4:
        ims_sales = ims_sales.withColumn(
            "ATC", func.substring(ims_sales.ATC, 0, 4)).distinct()
    ...
    if data_type == 'raw':
        path_all_brief = [extract_path + '/' + i +
                          "_rawdata_standard_brief" for i in project_all]
    elif data_type == 'max':
        path_all_brief = [extract_path + '/' + i +
                          "_max_standard_brief" for i in project_all]
    ...
    # 筛选,获取符合条件的项目和月份
    max_filter_list = max_standard_brief_all.where(
        (max_standard_brief_all.Date >= time_left) & (max_standard_brief_all.Date <= time_right))
    if project:
        max_filter_list = max_filter_list.where(
            max_filter_list.project.isin(project))
    if doi:
        max_filter_list = max_filter_list.where(max_filter_list.DOI.isin(doi))
    if atc:
        if max([len(i) for i in atc]) == 3:
            max_filter_list = max_filter_list.withColumn(
                "ATC", func.substring(max_filter_list.ATC, 0, 3)).distinct()
        elif max([len(i) for i in atc]) == 4:
            max_filter_list = max_filter_list.withColumn(
                "ATC", func.substring(max_filter_list.ATC, 0, 4)).distinct()
        max_filter_list = max_filter_list.where(max_filter_list.ATC.isin(atc))
    if molecule:
        max_filter_list = max_filter_list.where(
            max_filter_list['标准通用名'].isin(molecule))

    # 3. 注释项目排名
    max_filter_list = max_filter_list.join(
        project_rank, on="project", how="left").persist()
    ...
    # 分子最大月份数, 月份最全-得分
    months_max = report.groupby("标准通用名").agg(
        func.max("months_num").alias("max_month"))

    report = report.join(months_max, on="标准通用名", how="left")
    report = report.withColumn("drop_for_months", func.when(
        report.months_num == report.max_month, func.lit(0)).otherwise(func.lit(1)))

    # 对于raw_data 医院数量作为第二去重条件
    if data_type == 'raw':
        # 数据来源
        Source_window = Window.partitionBy(
            "project", "标准通用名").orderBy(func.col('Source'))
        rank_window = Window.partitionBy(
            "project", "标准通用名").orderBy(func.col('Source').desc())

        Source = max_filter_list.select("project", "标准通用名", 'Source').distinct() \
                                .select("project", "标准通用名",
                                        func.collect_list(func.col('Source')).over(
                                            Source_window).alias('Source'),
                                        func.rank().over(rank_window).alias('rank')).persist()
        Source = Source.where(Source.rank == 1).drop('rank')
        Source = Source.withColumn(
            'Source', func.concat_ws(',', func.col('Source')))

        report = report.join(
            Source, on=["project", "标准通用名"], how='left').persist()
        ...
        PHA_num = max_filter_list.select("project", "project_score", "标准通用名", "ATC", "PHA") \
            ...
            .persist()
        report = report.join(
            PHA_num, on=["标准通用名", "ATC", "project", "project_score"], how="left").persist()
        ...
        report = report.withColumn("drop_for_PHA", func.when(
            report.PHA_num == report.max_PHA_num, func.lit(0)).otherwise(func.lit(1)))
        report = report.withColumn("drop_for_PHA", func.when(
            report.drop_for_months == 0, report.drop_for_PHA).otherwise(None))
        ...
        report = report.withColumn("drop_for_score", func.when(
            report.project_score == report.max_score, func.lit(0)).otherwise(func.lit(1)))
        report = report.withColumn("drop_for_score", func.when(
            report.drop_for_PHA == 0, report.drop_for_score).otherwise(None))
    elif data_type == 'max':
        # 项目得分
        ...
        report = report.withColumn("drop_for_score", func.when(
            report.project_score == report.max_score, func.lit(0)).otherwise(func.lit(1)))
        report = report.withColumn("drop_for_score", func.when(
            report.drop_for_months == 0, report.drop_for_score).otherwise(None))
    ...
    time_range = time_range.withColumn("time_range", func.concat(
        time_range.min_time, func.lit("_"), time_range.max_time))
    ...
    if data_type == 'raw':
        ...
        report_a = report_a.orderBy(
            ["标准通用名", "months_num", "PHA_num", "project_score"], ascending=[0, 0, 0, 1])
    else:
        report_a = report_a.select("project", "ATC", "标准通用名", "time_range",
                                   "months_num", "drop_for_months", "project_score", "drop_for_score")
        report_a = report_a.orderBy(
            ["标准通用名", "months_num", "project_score"], ascending=[0, 0, 1])

    report_a = report_a.withColumn("flag", func.when(
        report_a.drop_for_score == 0, func.lit(1)).otherwise(func.lit(None)))
    # 输出report_a
    ...
    max_filter_path = max_filter_path.withColumn("path_month", func.concat(
        max_filter_path.path, func.lit("/Date_copy="), max_filter_path.Date))
    max_filter_path_month = max_filter_path.select(
        "path_month").distinct().toPandas()["path_month"].tolist()
    ...
    for eachpath in max_filter_path_month:
        ...
        if atc:
            if max([len(i) for i in atc]) == 3:
                df = df.withColumn(
                    "ATC", func.substring(df.ATC, 0, 3)).distinct()
            elif max([len(i) for i in atc]) == 4:
                df = df.withColumn(
                    "ATC", func.substring(df.ATC, 0, 4)).distinct()
    ...
    # 4. 注释项目排名
    max_filter_out = max_filter_raw.join(
        project_rank, on="project", how="left").persist()
    ...
    if data_type == 'max':
        ...
        max_filter_out = max_filter_out.withColumn(
            "p", max_filter_out.Units/max_filter_out.Predict_Unit)
        ...
        max_filter_out = max_filter_out.withColumn(
            "Sales", max_filter_out.Predict_Sales * max_filter_out.p)
        ...
        max_filter_out_1 = max_filter_out.where(
            max_filter_out["标准包装数量"].isNull())
        max_filter_out_2 = max_filter_out.where((~max_filter_out["标准包装数量"].isNull()) & (
            max_filter_out.Sales != 0) & (max_filter_out.Units != 0))
    ...
    if atc:
        ...
        time_range_act = time_range_act.withColumn("time_range", func.concat(
            time_range_act.min_time, func.lit("_"), time_range_act.max_time))
    ...
    molecule_names = out_extract_data.select("标准通用名").distinct().toPandas()[
        "标准通用名"].values.tolist()
    ...
    report_c = extract_sales.join(
        molecule_sales, on=["标准通用名", "ATC"], how="left")
    ...
    # 列名顺序调整
    report_c = report_c.select(
        "project", "ATC", "标准通用名", "time_range", "months_num", "Sales_rate")
