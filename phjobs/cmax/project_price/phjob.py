...
def execute(**kwargs):
    ...
    df_project_nation = readInFile(kwargs["df_project_nation"])
    # %%
    # =========== 函数定义 =============
    def qtrMonthMap():
    ...
    def getProjectPrice(df_project_nation, df_price_origin, df_price_city, df_price_province, df_price_year, df_price_pack, df_price_pack_year):
        ...
        df_project_price3 = df_project_price2.withColumn('quarter', func.substring(col('date'), 5, 2) ) \
                                            .withColumn('price', func.when(col('price').isNull(), col('price_city')).otherwise(col('price'))) \
