...
extract_other_universe -> hospital_other_fact （只包含SEG与PANEL）
...
def execute(**kwargs):
    ...
    _label = kwargs["label"]
    _hosp_other_fact_output = kwargs["hosp_other_fact_output"]
    ...
    fact_un_all = reduce(lambda x, y: x.union(y), list(map(fact_table, other_fact_mapping)))
    fact_un_all \
        .selectExpr("ID", "HOSPITAL_ID", "CATEGORY", "TAG", "VALUE", "COMPANY", "VERSION", "LABEL") \
        .write \
        .partitionBy("COMPANY", "VERSION") \
        .parquet(_hosp_other_fact_output, "append")
