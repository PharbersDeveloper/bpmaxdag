...
def execute(**kwargs):
    ...
    logger.info("ver_measures_content_path is {}.".format(ver_measures_content_path))
    ...
    df = spark.read.parquet(final_result_path) \
    ...
    for idx, row in df_lattices.iterrows():
        ...
        df_c.repartition(1).write.mode("append").parquet(ver_measures_content_path + "_temp")

    """
        应该分为两个job，或者在上面一次性计算
    """
    # 计算各种增长率
    df = spark.read.parquet(ver_measures_content_path + "_temp")
    df = df.withColumn("PROD_CUR_LEVEL", prod_level_udf(col("LATTLES"))) \
            .withColumn("GEO_CUR_LEVEL", geo_level_udf(col("LATTLES"))) \
            .withColumn("TIME_CUR_LEVEL", time_level_udf(col("LATTLES")))
    ...
    df.select(sch_columns).write.mode("overwrite").parquet(ver_measures_content_path)
