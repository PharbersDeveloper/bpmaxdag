...
def execute(**kwargs):
    ...
    _inputs = str(kwargs["input_weight_paths"]).replace(" ", "").split(",")
    _output = str(kwargs["output_weight_path"])
    _substr_tag = "v0.0.1-2020-06-08"
    _version = str(kwargs["version"])
    
    
    row = [{"PROVINCE": "NAN", "CITY": "NAN", "WEIGHT": "NAN", "DOI": "NAN" ,"PHA": "NAN"}]
    ...
    def get_df(path):
        ...
        readding = spark.read.parquet(path)
        cols = list(map(lambda x: x + " AS " + x.upper(), readding.columns))
        readding = readding.selectExpr(*cols).selectExpr("PROVINCE", "CITY", "DOI", "WEIGHT", "PHA")
        if readding.count() == 0:
