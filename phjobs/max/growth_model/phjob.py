...
def execute(**kwargs):
    ...
    if g_project_name != "Sanofi" and g_project_name != "AZ":
        ...
        # 完整年
        df_growth_rate_p1 = calculate_growth(df_raw_data.join(year_missing_df, on=["Year"], how="left_anti"))
        # 不完整年
        df_growth_rate_p2 = calculate_growth(df_raw_data.where(df_raw_data.Year.isin(g_year_missing + [y - 1 for y in g_year_missing] + [y + 1 for y in g_year_missing])), g_max_month)
