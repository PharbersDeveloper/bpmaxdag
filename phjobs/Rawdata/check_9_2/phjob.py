...
def execute(**kwargs):
    ...
    check_9_2 = Raw_data_1.groupBy('Date', '商品名').agg(func.sum(col('Sales')).alias('Sales')) \
                        .join(Raw_data_1.groupBy('Date').agg(func.sum('Sales').alias('Sales_month')), on='Date', how='left')
    check_9_2 = check_9_2.withColumn('share', col('Sales')/col('Sales_month')) \
    ...
    check_9_2 = lowerColumns(check_9_2)
    
    logger.debug('数据执行-Finish')
    
    return {'out_df':check_9_2}