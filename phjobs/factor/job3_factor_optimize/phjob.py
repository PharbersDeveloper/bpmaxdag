...
max_result_path = 's3a://ph-max-auto/v0.0.1-2020-06-08/Test/Eisai/MAX_201701-202009.csv'
...
factor = spark.read.parquet(factor1_path)


max_raw = max_result.where(col('DOI') == market)
max_df = max_raw.join(factor, on='City', how='left')
...
max_v = max_df.join(product_map.dropDuplicates(['min2']), max_df.Prod_Name==product_map.min2, how='left')

# %% max 数据
# 城市产品层面
max2 = max_v.groupBy('Brand', 'Citynew').agg(func.sum('Predict_Sales').alias('max_Prod')) \
            .withColumn('mkt', func.lit(market)) \
            .withColumnRenamed('Citynew', 'City')
# 全国的市场
max3 = max_v.groupBy('Citynew').agg(func.sum('Predict_Sales').alias('max_mkt')) \
            .withColumn('Market', func.lit(market)) \
            .withColumnRenamed('Citynew', 'City')


# %% panel数据
bll1_v = max_v.where(max_v.PANEL == 1)
# 城市产品层面
bll2 = bll1_v.groupBy('Brand', 'Citynew').agg(func.sum('Predict_Sales').alias('panel_Sales')) \
            .withColumn('mkt', func.lit(market)) \
            .withColumnRenamed('Citynew', 'City')
# 全国的市场
bll3 = bll1_v.groupBy('Citynew').agg(func.sum('Predict_Sales').alias('panel_mkt')) \
            .withColumn('Market', func.lit(market)) \
            .withColumnRenamed('Citynew', 'City')
            
# %% 确定IMS三个最大的产品
ims_info = ims_info.withColumn('Market', func.lit(market))

ims_inner = ims_info.withColumn('Brand_en', col('Prd_desc')) \
                    .join(product_map.select('Pack_ID', 'Brand').dropDuplicates(['Pack_ID']), on='Pack_ID', how='left')

ims_v1 = ims_inner.groupBy('Market', 'Brand', 'City').agg(func.sum('ims_poi_vol').alias('Prod')) \
                .withColumn('Brand', func.when(col('Brand').isNull(), func.lit('none')).otherwise(col('Brand')))

# 全国的商品的销量
ims_v1_part1 = ims_v1.where(col('City') == 'CHPA')
ims_v1_part1_v1 = ims_v1_part1.groupBy('Market', 'Brand').agg(func.sum('Prod').alias('Prod_CHPA'))
# other之外省份商品的销量
ims_v1_part2 = ims_v1.where(col('City') != 'CHPA')
ims_v1_part2_v1 = ims_v1_part2.groupBy('Market', 'Brand').agg(func.sum('Prod').alias('Prod_nonCHPA'))

# 
ims_v1_left = ims_v1_part1_v1.join(ims_v1_part2_v1, on=['Market','Brand'], how='left')
...
ims_v1_left1 = ims_v1_left.withColumnRenamed('Prod_CHPA', 'CHPA') \
                        .withColumnRenamed('Prod_nonCHPA', 'IMS_City') \
                        .withColumnRenamed('Prod_other', 'Prod') \
                        .select('Market','Brand','City','Prod')
# 每个城市的前三名产品
ims_v1_otherall = ims_v1_part2.union(ims_v1_left1)
...
ims_v1_mkt =  ims_v1_part2.union(ims_v1_left1) \
...
# ==== 
pdf = ims_panel_max.where(col('City')=='宁波市').toPandas()
...
factor1 = spark.read.parquet(factor1_path)
...
factor3.write.format("csv").option("header", "true") \
        .mode("overwrite").save(factor3_path)